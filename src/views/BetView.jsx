...
function MatchTimer({ match, t }) {
  ...
  return (
    <div style={{ color: timeLeft === 'LOCKED' ? 'var(--muted)' : 'var(--error)', fontWeight: 800, fontSize: '0.75rem' }}>
      {timeLeft === 'LOCKED' ? `🔒 ${t('pending')}` : `${t('lock_timer_prefix')} ${timeLeft}`}
    </div>
  );
}
...
function OngoingMatchCard({ match, votes, user, t }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ background: 'var(--teal)', padding: '0.6rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: 'white', fontWeight: 900, fontSize: '0.75rem', letterSpacing: '0.07em' }}>🔴 {t('live_bets_locked')}</span>
...
export default function BetView({ matches, votes, squadStats, user, handleVote, ongoingMatches, t }) {
              ...
              <div className="card-body" style={{ textAlign: 'center', paddingBottom: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    >
                      {myVote?.chosen_team === m.teams[0] ? `✅ ${t('picked')}` : t('bet')}
                    </button>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    >
                      {myVote?.chosen_team === m.teams[1] ? `✅ ${t('picked')}` : t('bet')}
                    </button>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' }}>
                      {squadMembers
                        .filter(member => {
